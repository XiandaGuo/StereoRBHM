...
class RBHMTrainer(Trainer):
    ...
    def eval_one_epoch(self, current_epoch):
        all_indexes = []
        metric_names = ['abs_h','rel']
        all_metrics = {'abs_h': [],'rel': []}
        ...
        for i, data in enumerate(self.val_loader):
            ...
            for batch_index in range(res.shape[0]):
                ...
                # all_metrics['abs_h'].append(abs(pred_h - data['height'][batch_index]))
                    ...
                    pred_h = 0.0
                gt_h = data['height'][batch_index].item()
                abs_h = abs(pred_h - gt_h)
                rel = abs_h / (gt_h + 1e-6)

                all_metrics['abs_h'].append(abs_h)
                all_metrics['rel'].append(rel)
